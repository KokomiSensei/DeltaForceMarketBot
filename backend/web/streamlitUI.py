...
from backend.bot.buyBot2 import BotConfig
...
def start_api_server():
    if not is_port_in_use(8000):
        ...
        try:
            ...
            subprocess.Popen([sys.executable, api_script_path], cwd=parent_dir, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            ...
            st.error(f"Failed to start API server: {e!s}")
            return False
    else:
...
if status:
    ...
    api_config = {
        "lowest_price": BotConfig.lowest_price,
        "volume": BotConfig.volume,
        "screenshot_delay": BotConfig.screenshot_delay,
        "debug_mode": BotConfig.debug_mode,
        "target_schema_index": BotConfig.target_schema_index,
    }
    running = False
...
def delete_config():
    ...
    st.session_state.config_profiles = [config for config in st.session_state.config_profiles if config.name != config_to_delete]
